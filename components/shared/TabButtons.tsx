    ...
    <div className="flex w-full bg-th-bkg-1 text-th-fgd-4">
      {values.map(([label, count], i) => (
        <div className={fillWidth ? 'flex-1' : ''} key={`${label}` + i}>
          <button
            className={`default-transition flex h-12 w-full items-center justify-center px-4 font-normal focus-visible:bg-th-bkg-2 md:px-6 ${
              ...
              label === activeValue
                ? label === 'buy'
                  ...
                  : 'bg-th-bkg-2 text-th-active focus-visible:text-th-active'
                : 'hover:cursor-pointer hover:text-th-fgd-2 focus-visible:text-th-fgd-1'
            }`}
            key={`${label}${i}`}
